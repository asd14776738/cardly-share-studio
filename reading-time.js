...
export function buildKicker({ platform = 'web', status = 'ok', readingMinutes, description = '', imageCount = 0 }) {
  ...
  if (platform === 'douyin') return kind + ' \u00b7 ' + (imageCount || 1) + ' MEDIA';
  if (!String(description).trim()) {
    return kind + (imageCount ? ' \u00b7 ' + imageCount + (imageCount === 1 ? ' IMAGE' : ' IMAGES') : ' \u00b7 NO PUBLIC TEXT');
  }
